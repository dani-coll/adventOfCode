...
rl.on('close', () => {
    ...
    for(let i = 0; i < lines.length; ++i) {
        if(lines[i][0] === lines[i][2] && lines[i][1] !== lines[i][3]) {
            ...
            let y = lines[i][1]
            matrix[x][y]++;
        } else if(belongsToDiagonal(lines[[i]])) {
...
function belongsToDiagonal(line) {
   let i = line[0];
   let j = line[1];

   let incX = line[2] > i ? 1: -1;
   let incY = line[3] > j ? 1: -1;

    while(i <= maxX && i >= 0) {
        i += incX;
        j += incY;

        if(line[2] === i && line[3] === j)  {
            matrix[i][j]++;
            while(i <= maxX && i >= 0) { // Go back to mark the points
                i -= incX;
                j -= incY;
                matrix[i][j]++;
